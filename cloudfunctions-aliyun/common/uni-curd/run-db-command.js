...
module.exports = function({
	...
	if (!rules[collection]) {
		throw new Error(`数据表${collection}未在rules字段内配置权限`)
	}
	...
	command.$db.forEach((stage, index) => {
		...
		if (method === 'lookup') {
			...
			if (!lookupColRule || !lookupColRule.read) {
				throw new Error(`lookup外联数据表${lookupCol}不可读取`)
			}
		}
		...
		// 如果使用了doc方法且设置了mixinCondition，将doc方法替换为where方法条件为doc条件且mixinCondition条件
		if (type !== 'create' && method === 'doc' && rule.mixinCondition) {
			exec = exec.where(db.command.and({
				_id: args[0],
			}, rule.mixinCondition))
		} else {
